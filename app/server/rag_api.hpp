...
inline RagRequest parse_rag_request(const nlohmann::json &request) {
    ...
    if (rag_request.mode != "sequential" &&
        ...
        throw std::invalid_argument("'mode' must be one of: sequential, hetero_parallel, carrier_baseline");
    }
...
inline nlohmann::json dump_rag_response(const RagResponse &response) {
    ...
    nlohmann::json decode_task_summaries_json = nlohmann::json::array();
    for (const auto &summary : response.decode_task_summaries) {
        decode_task_summaries_json.push_back({
